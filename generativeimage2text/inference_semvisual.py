...
def reshape_transform(tensor, height=14, width=14):
    ...
    # [batch_size, height, width, feature_dim]을 [batch_size, feature_dim, height, width]로 변경
...
def test_git_inference_single_image(image_path, model_name, prefix):
    ...
    target_layers = [model.image_encoder.transformer.resblocks[-1].ln_2]
    cam = GradCAM(model=model, target_layers=target_layers)
    img_path = '/home/supercone/ICAA/git/GenerativeImage2Text/aux_data/images/5.png'  # 이미지 경로
    img = load_image_by_pil(img_path)  # 당신의 이미지 로딩 함수를 사용
    transforms = get_image_transform(param)  # 당신의 이미지 변환 함수를 사용
    img = transforms(img).unsqueeze(0)  # 이미지 변환 및 배치 차원 추가

    targets = None
    grayscale_cam = cam(input_tensor=img, targets = targets)
    grayscale_cam = grayscale_cam[0, :]

    # 이미지에 GradCAM 적용 및 시각화
    to_pil_image = ToPILImage()
    rgb_img = to_pil_image(img.squeeze().cpu())
    cam_image = show_cam_on_image(np.array(rgb_img) / 255.0, grayscale_cam, use_rgb=True)

    # 시각화 이미지 저장
    cam_pil_img = ToPILImage()(torch.tensor(cam_image.transpose(2, 0, 1)))
    save_path = '/home/supercone/ICAA/git/GenerativeImage2Text/generativeimage2text/semin/grad_cam_result.jpg'
    cam_pil_img.save(save_path)
