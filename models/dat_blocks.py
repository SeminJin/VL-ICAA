...
class ShiftWindowAttention(LocalAttention):
    ...
    def forward(self, x):
        ...
        sw_x, _, _ = super().forward(shifted_x, self.attn_mask)#check
...
class DAttentionBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.offset_range_factor >= 0:
            self.sem_pos = offset + reference
            pos = torch.mul((offset + reference), gate)

        else:
            self.sem_pos = offset + reference.tanh()
            pos = (torch.mul((offset + reference), gate)).tanh()

        # self.sem_pos = torch.tanh(self.sem_pos)
        ...
        self.attn = attn
        # print("self.attn", self.attn.shape)
